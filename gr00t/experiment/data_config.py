...
class M2GMultiAgentDataConfig(BaseDataConfig):
    # 对应你 leRobot mapping 里的 video/state/action/annotation keys
    video_keys = ["video.main_camera_rgb"]
    state_keys = ["state.robot_joint_pos"]
    action_keys = ["action.action"]
    language_keys = ["annotation.task"]

    # 对齐你 1.6：video/state/language 只取当前帧；action horizon = 30
    observation_indices = [0]
    ...
    def transform(self) -> ModalityTransform:
        transforms = [
            ...
            GR00TTransform(
                ...
                action_horizon=len(self.action_indices),
                max_state_dim=64,
                max_action_dim=64,  # 你的 action 是 53 维（shape[53]），32 会不够
            ),
        ]
        return ComposedModalityTransform(transforms=transforms)
